            ...
            <div class="row-fluid">
                <div class="span9">
                    <div class="alert alert-info padTB20 relatedDept">
                        <h3 class="search_hospital relatedDept_right">共有 <span class="redTow"><c:if test="${doctors == null}">0</c:if> <c:if test="${doctors != null}">${doctors.size()}</c:if> </span> 位医生</h3>
                    </div>
                    <table class="table_big xijingtable xijingan relatedDept_list">
                        <c:if test="${doctors == null}"><h4>未安排医生</h4></c:if>
                        <c:if test="${doctors != null}">
                            <c:forEach items="${doctors}" var="doctor">
                                <tr>
                                    <td class="top wid25">
                                        <a>
                                            <c:if test="${doctor.sex.equals('男')}">
                                                <img src="${pageContext.request.contextPath}/img/0mandoctor.png" style="width:190px; height:140px" class="bor6 hospital-default-image"   />
                                            </c:if>
                                            <c:if test="${doctor.sex.equals('女')}">
                                                <img src="${pageContext.request.contextPath}/img/0womandoctor.png" style="width:190px; height:140px" class="bor6 hospital-default-image"   />
                                            </c:if>
                                        </a>
                                    </td>
                                    <td class="top wid25">
                                        <ul class="unstyled hospitalShow">
                                            <li><span class="muted">${doctor.doctorname} 医生</span></li>
                                            <li><span class="muted">职称：${doctor.academictitle}</span></li>
                                            <li><span class="muted">电话：${doctor.phonenum}</span></li>
                                        </ul>
                                    </td>
                                    <td class="wid10">
                                        <p><a href="${pageContext.request.contextPath}/doctor/showDoctorTime.do?doctorid=${doctor.id}" class="btn btn_red">预约挂号</a></p>
                                    </td>
                                </tr>
                            </c:forEach>
                        </c:if>
